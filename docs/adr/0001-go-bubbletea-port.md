...
Alternatives considered: keep Node/Ink (runtime + yarn friction; retired in favor of Go-only builds), Rust/Ratatui, shelling out to `git` (simpler but violates self-contained goal). Bubble Tea was chosen for parity with git-green and mature terminal UX primitives.
